...
import { Container, Card, Group, Avatar, Title, Text, Divider, Loader, Center, Button, Stack } from '@mantine/core';
          ...
          <Stack mt="md">
            ...
            {Object.entries(host.social_links).map(([platform, url]) => (
              <Text key={platform}>
                ...
                <a href={String(url)} target="_blank" rel="noopener noreferrer">{String(url)}</a>
              </Text>
            ))}
          </Stack>
        )}
      ...
      <Card shadow="sm" p="lg">
        ...
        {hostEvents.length === 0 ? (
          ...
          hostEvents.map(event => (
            <Card key={event.id} withBorder shadow="xs" my="sm" style={{ cursor: 'pointer' }} onClick={() => navigate(`/experience/${event.id}`)}>
              <Group justify="space-between">
                ...
                {event.photos?.[0] && <Avatar src={event.photos[0]} size={80} radius="md" />}
              </Group>
            </Card>
          ))
        )}
